...
void                        DASHReceiver::DownloadInitSegment    (IRepresentation* rep)
{
    ...
    if (initSeg)
    {
        initSeg->StartDownload(&this->m_ndnCon);
...
void			    DASHReceiver::UpdateThroughput(time_t beginTime, time_t endTime, uint32_t size)
{
    currentThroughput = new adaptation::ThroughputTrace();
    currentThroughput->beginTime = beginTime;
    currentThroughput->endTime = endTime;
    currentThroughput->segmentLength = size;
    printf("%ld %ld %d %d\n",currentThroughput->beginTime, currentThroughput->endTime, currentThroughput->segmentLength, currentThroughput->bitRate);
    double time = endTime-beginTime;
    if(time == 0)
      time == 1;
    currentThroughput->bitRate = (uint32_t)(size*8*1000/time);
      
}
  


ThroughputTrace* DASHReceiver::GetCurrentThroughput()
{
...
void*                       DASHReceiver::DoBuffering               (void *receiver)
{
	...
	beginTime = getCurrentTime();
        media->StartDownload(&dashReceiver->m_ndnCon);
	...
	dashReceiver->UpdateThroughput(beginTime, endTime, media->GetMediaSize());
