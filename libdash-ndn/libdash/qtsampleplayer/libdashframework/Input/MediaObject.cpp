...
bool		    MediaObject::StartDownload		(NDNConnection* con)
{
    this->segment->AttachDownloadObserver(this);
    con->PreDownload(this->segment);
//    this->size = con->m_dashConsumer.m_validateBufferSize;
    return this->segment->StartDownload(con);
}
...
uint32_t MediaObject::GetMediaSize()
{
    return size;
}
void MediaObject::SetMediaSize(uint32_t size)
{
    this->size = size;
}
